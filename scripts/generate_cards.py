...
def render_ascii_hero(emotion_ratios: dict[str, float] | None = None, y_start: float = 385) -> tuple[str, str, int]:
    ...
    card_inner = 800 * 0.55  # shrunk so ACTIVE REPOS panel can show >=3 entries
    char_width_at_1px = 0.6
    font_size = min(8.5, card_inner / (col_range * char_width_at_1px))
...
def render_emotion_legend(emotions, x, y_start, hero_height, ingest_ts=None):
    ...
    if ingest_ts:
        lines.append(
            f'  <text class="zg-secondary" x="{x}" y="{sub_y}" '
            f'font-family="\'TX-02\', \'Courier New\', Courier, monospace" font-size="8" '
            f'font-style="italic">based on Bluesky jetstream</text>'
        )
        lines.append(
            f'  <text class="zg-secondary" x="{x}" y="{sub_y + 10}" '
            f'font-family="\'TX-02\', \'Courier New\', Courier, monospace" font-size="8" '
...
def main():
    ...
    emotion_legend_svg = render_emotion_legend(
        JETSTREAM_EMOTIONS, x=700, y_start=hero_top - 6,
